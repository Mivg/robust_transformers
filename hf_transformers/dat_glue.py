# This file is based on my version of run_glue (which was forked version of transformers in hash 935e346959e81aa96a772c11d05734ea652932d1)
# And other then the changes listed there, it also uses the adversarial trainer as it's trainer object as well, uses my version of
# training_args and contain dumping of final prediction + updates on the results and the progress of the training
...
# because the released version of it in 4.0.1 misses the ingore_data_skip and model_paralel
from hf_transformers.training_args import TrainingArguments
...
from hf_transformers.adversarial_trainer import AdversarialTrainer

from hf_transformers.run_glue import _tsv_to_csv

task_to_keys = {
    "cola": ("sentence", None),
    "mnli": ("premise", "hypothesis"),  # note that our local mnli copy has sentence1 and sentence2 like snli
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, AdversarialTrainingArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, adv_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, adv_args = parser.parse_args_into_dataclasses()
    ...
    os.environ['TFHUB_CACHE_DIR'] = adv_args.tfhub_cache_dir
    ...
    if data_args.task_name is not None:
        ...
        if "sentence1" in non_label_column_names and "sentence2" in non_label_column_names:
            ...
            if 'sentence' in non_label_column_names:
                sentence1_key, sentence2_key = 'sentence', None
            elif len(non_label_column_names) >= 2:
                sentence1_key, sentence2_key = non_label_column_names[:2]
                if {'passage', 'question'} == set(non_label_column_names[:2]):
                    assert sentence1_key == 'question', 'The order of question first in boolq is not preserved'
            else:
